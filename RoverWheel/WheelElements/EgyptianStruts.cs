...
namespace Leap71
{
	...
	namespace Rover
	{
		public class EgyptianStruts : WheelElements
        {
			public EgyptianStruts(	WheelLayer  oLayer,
									uint		nSymmetry,
            ...
            public override Voxels voxConstruct()
			{
				...
				float dPhi						= (2f * MathF.PI) / (float)m_nSymmetry;
				...
				foreach (float fZ in aZList)
				{
					for (int i = 0; i < m_nSymmetry; i++)
					{
						...
						Vector3 vecPt3		= VecOperations.vecGetCylPoint(fRefInnerRadius, fPhi + 0.4f * dPhi, fZ);
						...
						aControlPoints.Add(vecPt3 + 0.1f * dR * vecRadial3);
						aControlPoints.Add(vecPt3 + 0.9f * dR * vecRadial3);
						aControlPoints.Add(vecPt1 + 0.7f * dR * vecRadial1);
						aControlPoints.Add(vecPt1 + 1.0f * dR * vecRadial1);
